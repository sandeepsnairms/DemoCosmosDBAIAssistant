...
public interface ICosmosDbService
{
    ...
    Task<List<Session>> GetSessionsAsync(PartitionManager.RecordQueryParams rParams);
    ...
    Task<List<Message>> GetSessionExistingMessagesAsync(PartitionManager.RecordQueryParams rParams, DateTime oldestMessageTimestamp);
    ...
    /// <param name="sessionId">Chat session identifier used to filter messsages.</param>
    /// <returns>List of chat message items for the specified session.</returns>
    Task<List<Message>> GetSessionMessageUpdatesAsync(PartitionManager.RecordQueryParams rParams, DateTime latestMessageTimestamp);

    /// <summary>
    /// Performs a point read to retrieve a single chat session item.
    /// </summary>
    /// <returns>The chat session item.</returns>
    Task<Session> GetSessionAsync(PartitionManager.RecordQueryParams rParams);
    ...
    Task<Session> InsertSessionAsync(PartitionManager.RecordQueryParams rParams,Session session);
    ...
    Task<Message> InsertMessageAsync(PartitionManager.RecordQueryParams rParams, Message message);
    ...
    Task<Message> UpdateMessageAsync(PartitionManager.RecordQueryParams rParams, Message message);
    ...
    /// <returns>Revised chat message item.</returns>
    Task<Message> UpdateMessageRatingAsync(PartitionManager.RecordQueryParams rParams, bool? rating);
    ...
    Task<Session> UpdateSessionAsync(PartitionManager.RecordQueryParams rParams, Session session);
    ...
    Task<Session> UpdateSessionNameAsync(PartitionManager.RecordQueryParams rParams, string name);

    /// <summary>
    /// Batch create or update chat messages and session.
    /// </summary>
    /// <param name="messages">Chat message and session items to create or replace.</param>
    Task UpsertSessionBatchAsync(PartitionManager.RecordQueryParams rParams,Session session,params dynamic[] messages);

    /// <summary>
    /// Batch deletes an existing chat session and all related messages.
    /// </summary>
    /// <param name="sessionId">Chat session identifier used to flag messages and sessions for deletion.</param>
    Task DeleteSessionAndMessagesAsync(PartitionManager.RecordQueryParams rParams);
